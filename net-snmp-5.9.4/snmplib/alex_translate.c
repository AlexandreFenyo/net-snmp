...
extern int alex_main_translate(int argc, char *argv[]);

// Must be in sync with SNMPManager.swift
#define ALEX_TRANSLATE_IN_LEN 1024
#define ALEX_TRANSLATE_OUT_LEN 8192

char alex_translate_in[ALEX_TRANSLATE_IN_LEN];
char alex_translate_out[ALEX_TRANSLATE_OUT_LEN];
char *alex_translate_av[3] = { "snmptranslate", "-Td", alex_translate_in };

void alex_translate(char *str) {
    // Restart parsing args for each call to alex_translate.
    optind = 1;

    alex_translate_out[0] = 0;

    strncpy(alex_translate_in, str, ALEX_TRANSLATE_IN_LEN - 1);
    alex_translate_in[ALEX_TRANSLATE_IN_LEN - 1] = 0;

    alex_main_translate(sizeof alex_translate_av / sizeof *alex_translate_av, alex_translate_av);
}

void alex_get_translation(char *target) {
    stpcpy(target, alex_translate_out);
}
...
int
alex_main_translate(int argc, char *argv[])
{
    int             arg;
    ...
    do {
        ...
        if (print == 1) {
            ...
            if (description) {
                ...
                snprint_description(alex_translate_out, sizeof alex_translate_out, name, name_length, width);
#endif /* NETSNMP_DISABLE_MIB_LOADING */
            }
        }
        current_name = argv[++optind];
